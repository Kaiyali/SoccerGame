...
void setup() {
  ...
  circleRadius = 100;
  gameState = 0;
   ArgentinaBall  = new Player(width/2+width/6, height/2, circleRadius);
   soccerBall     = new Player(width/2, height/2, circleRadius, "soccerBall.png");
   FrenchBall     = new Player(width/3, height/2, circleRadius);
 
  while(gameState == GameStates.FIELDSTATE){
